...
def test_minimal_sampling_order_in_plane(NA, n_medium, max_pos, wavelength):
    obj = Objective(NA=NA, focal_length=4e-3, n_bfp=1.0, n_medium=n_medium)
    assert obj.minimal_sampling_order(
        (np.ones(1) * max_pos, np.zeros(1), np.zeros(1)), wavelength
    ) == max(
...
@pytest.mark.parametrize("max_pos", np.linspace(0.0, 100e-6, 21))
def test_minimal_sampling_order_out_of_plane(NA, n_medium, max_pos, wavelength):
    obj = Objective(NA=NA, focal_length=4e-3, n_bfp=1.0, n_medium=n_medium)
    ...
    if abs(max_pos) > 0.0 and (
        ...
        N = 2
    assert N == obj.minimal_sampling_order((0, 0, max_pos), wavelength)
...
def test_minimal_sampling_order_raises():
    ...
    with pytest.raises(ValueError, match="Unsupported method: unsupported"):
        obj.minimal_sampling_order(np.zeros((3, 1)), lambda_vac=500e-9, method="unsupported")
    ...
    for coordinates in [(0,), (0, 0), (0, 0, 0, 0)]:
        with pytest.raises(
            ...
            obj.minimal_sampling_order(coordinates, lambda_vac=500e-9)
