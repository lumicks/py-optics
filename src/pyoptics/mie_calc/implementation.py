    ...
    for m in range(n_pupil_samples):
        for p in range(n_pupil_samples):
            if not farfield_data.aperture[p, m]:  # Skip points outside aperture
                continue
            matrices = [
                R_th(farfield_data.cos_theta[p,m], farfield_data.sin_theta[p,m]) @ 
                R_phi(farfield_data.cos_phi[p,m], -farfield_data.sin_phi[p,m]),
                R_phi(0, -1) @ 
                R_th(farfield_data.cos_theta[p,m], farfield_data.sin_theta[p,m]) @ 
                R_phi(farfield_data.cos_phi[p,m], -farfield_data.sin_phi[p,m])
            ]
...
@njit(cache=True)
def scattered_field_fixed_r(an: np.ndarray, bn: np.ndarray, krh: np.ndarray, 
...
@njit(cache=True)
def internal_field_fixed_r(cn: np.ndarray, dn: np.ndarray, 
...
@njit(cache=True)
def scattered_H_field_fixed_r(an: np.ndarray, bn: np.ndarray, krh: np.ndarray, 
                            dkrh_dkr: np.ndarray, k0r: np.ndarray, 
                            alp: np.ndarray, alp_sin: np.ndarray,
                            alp_deriv: np.ndarray,
                            cos_theta: np.ndarray, cosP: np.ndarray,
...
@njit(cache=True)
def internal_H_field_fixed_r(cn: np.ndarray, dn: np.ndarray, 
    sphBessel: np.ndarray, jn_over_k1r: np.ndarray, jn_1: np.ndarray, 
    alp: np.ndarray, alp_sin: np.ndarray, alp_deriv: np.ndarray,
    cos_theta: np.ndarray, cosP: np.ndarray, sinP: np.ndarray, 
